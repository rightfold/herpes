...
import Herpes.UseCase (UseCase (..), UseCaseId, _UseCaseId)
...
makeToolbar :: (UseCaseId -> IO ()) -> IO Gtk.Widget
makeToolbar callback = do
  ...
  _ <- Gtk.on useCaseGoButton Gtk.buttonActivated $ do
    ...
    case rawUseCaseId ^? _UseCaseId of
...
example = do
  ...
  workspaceWidget <- renderWorkspace $
    HashMap.singleton ("MOTD" ^?! _UseCaseId) (UseCase (Motd.screen getMessages))
